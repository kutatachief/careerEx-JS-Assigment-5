// This code defines an array of objects representing a list of drugs.

// The array contains a list of drugs with their details.
...
function getDrugsByCategory(category) {
    return drugs.filter(drug => drug.category.toLowerCase() === category.toLowerCase());
  }
const analgesics = getDrugsByCategory("Analgesic");
...
function countDrugsByManufacturer(manufacturer) {
    return drugs.filter(drug => drug.manufacturer.toLowerCase() === manufacturer.toLowerCase()).length;
  }
const pfizerCount = countDrugsByManufacturer("Pfizer");
console.log(`Number of drugs from Pfizer: ${pfizerCount}`);
